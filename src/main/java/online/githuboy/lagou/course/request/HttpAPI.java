...
public class HttpAPI {
    ...
    public static AliyunVodPlayInfo getVodPlayerInfo(String rand, String playAuth, String fileId, String formats) {
        ...
        String response = HttpUtils.getWithoutEncode(playInfoRequestUrl).execute().body();
        ...
        if (playInfos != null && playInfos.size() > 0) {
            List<AliyunVodPlayInfo> candidates = playInfos.toJavaList(AliyunVodPlayInfo.class);
            // Prefer Normal + m3u8 + highest bitrate if available
            AliyunVodPlayInfo best = null;
            for (AliyunVodPlayInfo info : candidates) {
                if (info == null || info.getPlayURL() == null) {
                    continue;
                }
                ...
                if (!info.getPlayURL().contains("m3u8")) {
                    continue;
                }
                if (best == null) {
                    best = info;
                ...
                long bestBitrate = parseBitrate(best.getBitrate());
                long currBitrate = parseBitrate(info.getBitrate());
    ...
    private static long parseBitrate(String bitrate) {
        ...
        try {
            return (long) Double.parseDouble(bitrate);
